function [metric] = metricGet_LentzComoving(gridSize, worldCenter, v, scale, gridScale)
%% METRICGET_LENTZCOMOVING: Builds the Lentz metric in Galilean comoving frame
%
%   INPUTS: 
...
if gridSize(1) > 1
    error('The time grid is greater than 1, only a size of 1 can be used for Lentz');
end
...
for i = 1:gridSize(2)
    for j = 1:gridSize(3)
        for k = 1:gridSize(4)
            ...
            % Assign dxdt term
            beta{1}(t,i,j,k) = v*(1-WFX);

            % Assign dydt term
            beta{2}(t,i,j,k) = v*WFY;
        end
    end
end
